...
def action(j, x):
    jp = (j + 1) % N
    jm = (j - 1) % N
    return a * harmonic_potential(x[j]) + x[j] * (x[j] - x[jp] - x[jm]) / a
...
def MCaverage(x, G, N_config): 
    ...
    for j in range(0, 5 * N_cor):  #thermalization
        update(x)
    for alpha in range(0, N_config):
        for j in range(0, N_cor):  #erasing correlations
            update(x)
        for n in range(0, N):
...
def binning(data, bin_size):
    N_cf, N = data.shape
    N_bin = N_cf // bin_size #some configurations might be discarded
    binned = np.zeros((N_bin, N))  #initialization of the array that will contain the avarages
    for i in range(N_bin):
        binned[i] = np.mean(data[i * bin_size:(i + 1) * bin_size], axis=0)  #for each block (i) avarages of config. are calculated 
    return binned
...
def analysis(N_cf_list):
    for N_config in N_cf_list:
        ...
        plt.axhline(y=1.0, color='black', linestyle='--', linewidth=1.0, label='Exact result (1.0)')
        ...
        plt.title(f'Harmonic Potential (with binned and bootstrapped data): N={N}, a={a}, N_config={N_config}, N_cor={N_cor}')
        plt.xlabel('t')
