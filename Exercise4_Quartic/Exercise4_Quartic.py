...
def binning(data, bin_size):
    N_cf, N = data.shape
    N_bin = N_cf // bin_size
    binned = np.zeros((N_bin, N))
    for i in range(N_bin):
        binned[i] = np.mean(data[i * bin_size:(i + 1) * bin_size], axis=0)
    return binned
...
def analysis(N_cf_list):
    for N_config in N_cf_list:
        ...
        plt.title(f'Quartic Potential (with binned and bootstrapped data): N={N}, a={a}, N_config={N_config}, N_cor={N_cor}')
        plt.xlabel('t')
        ...
        filename = f"E4_Quartic_Nconfig{N_config}.png"
