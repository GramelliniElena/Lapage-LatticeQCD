...
use_improved_action = True  #toggle for improved discretization
...
@njit
def action(j, x, use_improved_action):
    jp1 = (j + 1) % N
    jm1 = (j - 1) % N
    jp2 = (j + 2) % N
    jm2 = (j - 2) % N

    if use_improved_action:
        # Improved kinetic term
         kinetic = (1 / (12 * a)) * x[j] * (x[jm2] - 16 * x[jp1] + 15 * x[j] - 16 * x[jm1] + x[jp2])

    else:
        # Standard kinetic term 
        kinetic = x[j] * (x[j] - x[jp1] - x[jm1]) / a

    potential = a * harmonic_potential(x[j])
...
def update(x):
    for j in range(N):
        old_x = x[j]
        old_action = action(j, x, use_improved_action)
        x[j] += np.random.uniform(-eps, eps)  #add a random number in (-eps, eps) to x[j]
        d_action = action(j, x, use_improved_action) - old_action
...
def analysis(N_cf_list):
    for N_config in N_cf_list:
        print(f"Simulazione con N_config = {N_config} (improved: {use_improved_action})...")
